...
// curl -F "title=TestProject" -F "bundleId=com.test.testproject" -F "bundleVersion=1.0" -F "uploadKey=secretkey" -F "fileToUpload=@TestProject.ipa" https://exampleserver.com/
...
function handleUpload() {
	...
	if ($uploadOk) {
		if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $ipaFilePath)) {
          ...
          $status .= '<br/><br/>';
          $status .= '<img src="https://chart.googleapis.com/chart?chs=100x100&cht=qr&chl='.$_SERVER["SERVER_NAME"].''.strtok($_SERVER["REQUEST_URI"], '?').'?c='.$randomToken.'" title="Link to install" />';
...
function showPageWithList($tokenFilter) {
	...
	echo '<h3>Nitin Bhatia</h3></center></body></html>';
}
